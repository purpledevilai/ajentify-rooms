...
const RegularChatRoom = observer(() => {
    ...
    useEffect(() => {
        ...
        regularChatRoomStore.initialize(roomId);
     ...
     */
    useEffect(() => {
        if (localVideoRef.current && regularChatRoomStore.mediaStream) {
            localVideoRef.current.srcObject = regularChatRoomStore.mediaStream;
        }
    }, [regularChatRoomStore.mediaStream]);

    const handleLeave = () => {
        regularChatRoomStore.leaveRoom();
    ...
    return (
        <Box position="relative" height="100vh" width="100vw" overflow="hidden" bg={bg} color={text}>
            ...
            >
                ...
                {/* Peer Video Elements */}
                {regularChatRoomStore.roomConnection && <PeerVideoElements roomConnection={regularChatRoomStore.roomConnection}/>}
            </Flex>
                    ...
                    <HStack spacing={2}>
                        <Tooltip label="Mute / Unmute Microphone">
                            <IconButton
                                ...
                                icon={regularChatRoomStore.audioMuted ? <FaMicrophoneSlash /> : <FaMicrophone /> }
                                onClick={() => regularChatRoomStore.toggleMediaDeviceMute("audio")}
                            />
                        </Tooltip>
                        <Select
                            placeholder="Select Microphone"
                            value={regularChatRoomStore.selectedAudioDevice?.deviceId ?? ""}
                            onChange={(e) => regularChatRoomStore.setMediaDevice(e.target.value, "audio")}
                            maxW="200px"
                        >
                    ...
                    <HStack spacing={2}>
                        <Tooltip label="Toggle Camera">
                            <IconButton
                                ...
                                icon={regularChatRoomStore.videoMuted ? <FaVideoSlash /> : <FaVideo />}
                                onClick={() => regularChatRoomStore.toggleMediaDeviceMute("video")}
                            />
                        </Tooltip>
                        <Select
                            placeholder="Select Camera"
                            value={regularChatRoomStore.selectedVideoDevice?.deviceId ?? ""}
                            onChange={(e) => regularChatRoomStore.setMediaDevice(e.target.value, "video")}
                            maxW="200px"
                        >
...
export default RegularChatRoom;
